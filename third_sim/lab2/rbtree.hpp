...
template <typename T>
typename RedBlackTree<T>::Node* RedBlackTree<T>::rbLookup(Node* root, T* key)
{
    if (root == nullptr)
        return root;
...
template <typename T>
void RedBlackTree<T>::search(T key) {
    Node* node = rbLookup(root, &key);
}
